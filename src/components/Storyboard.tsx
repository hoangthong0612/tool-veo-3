...
const Storyboard: React.FC<StoryboardProps> = ({ scenes, onDownloadAll }) => {
    const hasNarration = scenes.length > 0 && scenes.some(scene => scene.narration);
        ...
        <div className="bg-gray-800 p-6 rounded-lg shadow-xl">
            ...
            <div className="space-y-4">
                <div className="hidden lg:grid grid-cols-12 gap-4 font-bold text-gray-400 p-2">
                    <div className={hasNarration ? "col-span-2" : "col-span-3"}>Image Prompt</div>
                    <div className="col-span-3">Generated Image</div>
                    <div className={hasNarration ? "col-span-2" : "col-span-3"}>Video Prompt</div>
                    <div className="col-span-3">Generated Video</div>
                    {hasNarration && <div className="col-span-2">Narration</div>}
                </div>
                {scenes.map(scene => (
                    <div key={scene.sceneNumber} className="grid grid-cols-1 lg:grid-cols-12 gap-4 bg-gray-900 p-4 rounded-lg border border-gray-700 items-center">
                        <div className={hasNarration ? "col-span-1 lg:col-span-2" : "col-span-1 lg:col-span-3"}>
                        ...
                        {hasNarration && (
                            <div className="col-span-1 lg:col-span-2">
                                <h4 className="font-bold lg:hidden text-gray-400 mb-1">Narration</h4>
                                <p className="text-sm text-gray-300">{scene.narration}</p>
                            </div>
                        )}
                    </div>
                ))}
